function Enable-StmScheduledTask {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        Windows Task Scheduler. This function wraps the built-in Enable-ScheduledTask cmdlet to provide credential
        ...
        2. Enables the specified scheduled task
        3. Verifies that the task has been successfully enabled
    ...
    .PARAMETER TaskName
        Specifies the name of the scheduled task to enable. This parameter is mandatory and must match the exact
        name of the task as it appears in the Task Scheduler.

    .PARAMETER TaskPath
        Specifies the path of the scheduled task to enable. The task path represents the folder structure in the
        Task Scheduler where the task is located (e.g., '\Microsoft\Windows\PowerShell\'). If not specified, the
        root path ('\') will be used.

    .PARAMETER ComputerName
        Specifies the name of the computer on which to enable the scheduled task. If not specified, the local
    ...
    .PARAMETER PassThru
        Returns an object representing the enabled scheduled task. By default, this cmdlet does not generate any
        output.
    ...
    begin {
        Write-Verbose 'Starting Enable-StmScheduledTask'
    ...
    process {
        if ($PSCmdlet.ShouldProcess("$TaskName at $TaskPath on $ComputerName", 'Enable scheduled task')) {
            try {
                Write-Verbose "Enabling scheduled task '$TaskName' at path '$TaskPath' on computer '$ComputerName'..."
                $enableScheduledTaskParameters = @{
                    TaskName    = $TaskName
                    TaskPath    = $TaskPath
                    CimSession  = $cimSession
                    ErrorAction = 'Stop'
                }
                Enable-ScheduledTask @enableScheduledTaskParameters

                Write-Verbose "Verifying that scheduled task '$TaskName' has been enabled..."
                ...
                if ($task.State -ne 'Disabled') {
                    $successMsg = (
                        ...
                        "' has been successfully enabled on computer '" + $ComputerName + "'."
                    )
                    ...
                    Write-Error (
                        "Scheduled task '$($TaskName)' was not enabled successfully. " +
                ...
                $errorRecordParameters = @{
                    ...
                    ErrorId           = 'ScheduledTaskEnableFailed'
                    ErrorCategory     = [System.Management.Automation.ErrorCategory]::NotSpecified
                    TargetObject      = $TaskName
                    Message           = (
                        "Failed to enable scheduled task '$TaskName' at path '$TaskPath' on computer " +
                        "'$ComputerName'. {$_}"
                    )
                    RecommendedAction = (
