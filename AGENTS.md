...
- Use `ValidateNotNullOrEmpty` instead of generic `ValidateNotNull`
- Implement `ValidateScript` for complex validation logic
- Use `ValidatePattern` for string format validation
...
```powershell
# ✅ CORRECT: Parameter name on separate line with blank line separation
# Multi-argument attributes on separate lines
[Parameter(
    Mandatory = $true,
...
# ❌ INCORRECT: Parameter name on same line as type, no separation
# Multi-argument attributes on single line
...
5. After PR approval and merge to `main`, the publishing workflow automatically runs
6. Create a git tag and GitHub release (see Git Tagging section below)
7. Monitor GitHub Actions tab to verify successful publication
...
4. Create a GitHub release from the tag with release notes:
   ...
   ```powershell
   gh release create v1.2.3 --title "v1.2.3" --notes-from-tag
   ```
...
### Pre-Commit Requirements for AI-Generated Code

All AI-generated code must pass these checks before suggestion:

1. **PSScriptAnalyzer Compliance**: No violations of project analyzer rules
2. **Build System Validation**: Code must work with `./build.ps1 -Task Build`
3. **Test Coverage**: Comprehensive test coverage for new functions
4. **Documentation Standards**: Complete comment-based help with examples

### Quality Gates for AI Agents

**Validation Process:**

- Use `./build.ps1 -Task Test` to validate all changes
- Ensure markdown documentation passes linting using VS Code markdownlint extension
- Verify integration with existing module structure  
- Test functions interactively to ensure they work as expected

**Code Standards Enforcement:**

- **Function Structure**: Match established patterns in existing Public/ functions
- **Parameter Design**: Use consistent validation attributes and naming conventions
- **Error Handling**: Implement the same error handling patterns used throughout the module
- **Documentation**: Follow the exact comment-based help format used in existing functions
- **Testing**: Generate tests that follow the project's test organization and mocking patterns

### AI Agent Validation Workflow

1. **Pattern Analysis**: Examine similar existing functions before code generation
2. **Code Generation**: Create code following established project patterns
3. **Interactive Testing**: Test the generated function manually to verify behavior
4. **Build Validation**: Run `./build.ps1 -Task Test` to ensure all quality checks pass
5. **Integration Verification**: Confirm the function integrates properly with the module

## AI Agent Troubleshooting Guide

### Common AI Development Issues and Solutions
...
```powershell
# ❌ AVOID: Generic parameter validation
[ValidateNotNull()]
[string]$TaskName

# ✅ PREFER: Specific validation for scheduled task names
[ValidateNotNullOrEmpty()]
[ValidatePattern('^[^\\/:*?"<>|]*$')]  # No invalid filename characters
[string]
$TaskName
...
# Symptom: "Module could not be imported"
# Solution: Use build system, not direct imports
./build.ps1 -Task Build  # Ensures proper dependency resolution

# Issue: Test Execution Failures
# Symptom: Tests pass individually but fail in suite
# Solution: Always use build system for testing
./build.ps1 -Task Test    # Provides consistent test environment

# Issue: Clean Build Required
# Symptom: Inconsistent behavior or cached module state
# Solution: Clean and rebuild
./build.ps1 -Task Clean
./build.ps1 -Task Build
```

**Cluster Environment Issues:**

```powershell
# Issue: CIM Session Failures
# Symptom: "Access denied" or "RPC server unavailable"
# Solution: Verify credentials and network connectivity
New-CimSession -ComputerName $ClusterNode -Credential $Credential -ErrorAction 'Stop'

# Issue: Cluster cmdlet not available
# Symptom: "The term 'Get-ClusterResource' is not recognized"
# Solution: Install Failover Clustering PowerShell module
...
# Symptom: Debugger doesn't attach or breakpoints ignored
# Solution: Use proper VS Code tasks and configurations
# Check .vscode/launch.json and .vscode/tasks.json are properly configured

# Issue: PowerShell Extension Problems
# Symptom: IntelliSense not working, syntax highlighting broken
# Solution: Reload VS Code extension or restart
Ctrl+Shift+P -> "PowerShell: Restart Session"
```

### AI Agent Debugging and Development Workflow
